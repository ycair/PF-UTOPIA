...
| 11 | 體力恢復劑 | consumable | common | — | 100 | 🧃 | 商店購買（安幣） |
| 12 | 強效體力恢復劑 | consumable | uncommon | — | 300 | 🧋 | 商店購買（安幣） |
...
|------|-----|
| 基礎安幣 | 100 元/天 |
| 連續簽到加成 | +20 元/天（第 N 天 = 100 + (N-1)×20） |
| 黃金月卡逸幣 | +20 逸幣/天 |
| 鑽石月卡逸幣 | +50 逸幣/天 |
| 中斷簽到 | 連續天數重置為 1 |
...
| 道具 | 價格 | 貨幣 | 效果 |
|------|------|------|------|
| 體力恢復劑 | 100 | 安幣 | 恢復 50 體力 |
| 強效體力恢復劑 | 300 | 安幣 | 恢復 150 體力 |
| 線香 | 50 | 安幣 | 膜拜消耗品 |
| 糖果 | 30 | 安幣 | 供奉精靈 |
| 轉法輪 | 500 | 安幣 | 增加修為 |
...
- **顯示項目**：狀態（能力評分/體力/元神/修為）、活躍（聊天等級/經驗值）、戰力（攻/防/血/能力點）、貨幣（5 種）
- **進度條**：20 格文字進度條 (█ + ░)
- **不註冊可查看**：🔴 提示註冊
...
- **限制**：每日一次（UTC+8 午夜重置）
- **獎勵**：基礎 100 安幣 + 連續簽到加成
- **月卡**：黃金+20 逸幣 / 鑽石+50 逸幣

### 5.4 背包 `/bag`
- **顯示**：所有道具列表（名稱、數量、emoji）
- **分類**：材料 / 消耗品

### 5.5 野區探索 `/explore <zone>`
- **zone 選項**：初始草原外圍、初始草原內部、翡翠森林外圍、翡翠森林內部、沿海小徑
- **流程**：扣體力 → 隨機遇敵 → 回合制戰鬥 → 顯示最後 5 回合記錄 → 結算掉落
- **體力不足**：提示使用 `/use 體力恢復劑`

### 5.6 副本挑戰 `/dungeon <boss>`
- **boss 選項**：入侵殭屍、入侵軍團、女僕的緞帶、女僕們的緞帶
- **戰力門檻**：低於建議 70% 無法挑戰
- **失敗**：顯示「🔴 戰鬥失敗，建議提升戰力」

### 5.7 主線祭壇 `/altar <chapter>`
- **章節**：1~15（可擴充）
- **每章**：固定 Boss 強度遞增
...
### 6.1 商店 `/shop [category]`
- **category**：materials / consumables / all
- **顯示**：價格（材料=浮動賣價 / 消耗品=固定買價）、emoji、描述

### 6.2 購買 `/shop_buy <item> <quantity>`
- **貨幣**：安幣
- **適用**：消耗品
- **檢查**：安幣是否足夠

### 6.3 出售 `/shop_sell <item> <quantity>`
- **貨幣**：托幣
- **適用**：材料
- **價格**：使用當前 shop_sell_prices.current_price

### 6.4 使用道具 `/use <item>`
- **效果**：體力恢復劑 → +50 體力；強效 → +150；線香 → +1 線香
- **限制**：需持有該道具

### 6.5 彩券購買 `/lottery_buy <n1>~<n6>`
- **號碼範圍**：1~49，不重複
- **價格**：500 安幣

### 6.6 自動選號 `/lottery_auto`
- **價格**：500 安幣
- **隨機**：6 不重複號碼

### 6.7 VIP 選號 `/lottery_vip`
- **價格**：1,000 安幣
- **效果**：中獎獎金 ×2

### 6.8 彩券兌獎 `/lottery_check`
- **顯示**：本期開獎號碼 + 你的中獎情況
