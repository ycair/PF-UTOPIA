...
| invest | 997110126261588009 | 💰投資交易所 | 投資交易所 |
...
| `vol_down` REAL | 跌幅上限 |
| `vol_up` REAL | 漲幅上限 |
| `bankrupt` BOOLEAN | 是否破產（≤19 元） |

| 代碼 | 名稱 | 初始價 | 漲跌範圍 | emoji |
|------|------|--------|---------|-------|
| bronze_turtle | 青銅烏龜 | 1,000 | -10% ~ +10% | 🐢 |
| silver_turtle | 白銀烏龜 | 10,000 | -10% ~ +10% | 🐢 |
| gold_turtle | 黃金烏龜 | 100,000 | -10% ~ +10% | 🐢 |
| diamond_turtle | 鑽石烏龜 | 500,000 | -10% ~ +10% | 💎 |
| stone_turtle | 石頭烏龜 | 800 | -10% ~ +10% | 🪨 |
| ruby_turtle | 紅寶石烏龜 | 1,200,000 | -20% ~ +30% | 💠 |

##### 投資規則
- 每日 8:00、12:00、16:00 三次報價
- 紅寶石烏龜漲跌 -20% ~ +30%，其餘 -10% ~ +10%
- 股價跌至 19 元即破產，歸零
- 使用托幣交易
...
| ID | 名稱 | 類型 | 安全 | 特殊效果 |
|----|------|------|:----:|---------|
| 1 | 烏托邦主城 | capital | ✅ | 出生點、回城滿血 |
| 2 | 競技場 | arena | ✅ | — |
| 3 | 彩券中心 | town | ✅ | — |
| 4 | 投資交易所 | town | ✅ | — |
| 5 | 女僕教堂 | sanctuary | ✅ | 打坐掛機 |
| 6 | 初始草原 | wild | ❌ | 唯一出城閘道 |
| 7 | 翡翠森林 | wild | ❌ | 交通樞紐 |
| 8 | 沿海小徑 | wild | ❌ | — |
| 9 | 搗蛋精靈之森 | wild | ❌ | — |
| 10 | 世界魔皇巢穴 | dungeon | ❌ | 世界魔皇討伐 |
| 11 | 大士爺廟 | temple | ✅ | 鬼王庇佑、膜拜 |
| 12 | 寵物天堂 | wild | ❌ | 寵物戰鬥 |
| 13 | 舊城邦 | dungeon | ❌ | ☢️ 輻射：防禦 -50% |
| 14 | 山林後的花園 | dungeon | ❌ | 女僕 Boss 討伐 |
| 15 | 冒險者公會 | town | ✅ | 每日簽到、任務板、等級認證 |
...
#### `node_prices` — 節點差異化收購價

| 規則 | 說明 |
|------|------|
| 主城 | 全物資基礎價 100% |
| 野外 | 僅收購該節點怪物會掉落的物資，最高 30% 主城價 |
| 安全節點 | 沿用主城價格 |
...
⚠️ 翡翠森林探索完外圍後，有 15% 機率觸發「迷路」深入內部。
...
| 副本 Key | 名稱 | 體力 | 建議戰力 | 完美戰力 | HP | ATK | DEF | 所在節點 | 掉落 |
|----------|------|------|---------|---------|-----|-----|-----|---------|------|
| zombie | 征服入侵殭屍 | 10 | 2,800 | 3,000 | 120 | 10 | 5 | 🏚️ 舊城邦 | 骨頭(3~6,80%), 魔石(1~2,40%), 線香(1~3,60%) |
| army | 征服入侵軍團 | 30 | 24,300 | 27,000 | 400 | 25 | 15 | 🏚️ 舊城邦 | 皮革(3~6,70%), 魔石(2~4,50%), 線香(2~5,60%) |
| maid_ribbon | 解開女僕的緞帶 | 50 | 218,700 | 243,000 | 2,000 | 60 | 40 | 🌸 山林後的花園 | 緞帶(1~3,60%), 魔石(3~7,50%), 轉法輪(1~2,30%) |
| maid_ribbons | 解開女僕們的緞帶 | 70 | 1,749,600 | 2,187,000 | 8,000 | 150 | 100 | 🌸 山林後的花園 | 緞帶(3~8,70%), 魔石(5~12,50%), 轉法輪(2~5,40%) |
...
| 參數 | 值 |
|------|-----|
| 節點限制 | 必須位於女僕教堂（`current_node` 檢查 + 頻道檢查雙重驗證） |
| 打坐期間限制 | 不可 `/move`、不可戰鬥、不可交易、體力不回復 |
| 結束方式 | `/meditate_stop` |
| 托幣獎勵 | ~60 托幣/小時 |
| 戰利品 | 隨機掉落（初始草原掉落池，期望值 = 體力刷副本收益 50%） |
| 元神經驗 | 10 EXP/小時 |
| 元神升級 | 每級需 (等級+1)×50 EXP，升級 HP+5、體力上限+5 |
| 線香 | 領取後出現在背包，膜拜時自動扣除，不可 `/use` |
| 轉法輪 | 商店可購，使用後元神 EXP+10 + 修為進度+3 |
...
| 攻擊 buff | +50%，持續 30 分鐘 |
| 修為進度 | +1/次 |
| 修為升級 | 公式：`(等級+1) × 3 × (1 + ln(等級+1) × 0.5)`，升級能力點+1 |
| 轉法輪 | 消耗品（商店可購），使用後元神 EXP+10 + 修為進度+3 |
...
| 位置類型 | 節點 | 材料出售 | 消耗品購買 |
|---------|------|---------|-----------|
| 🏰 城內 | 主城、競技場、彩券中心、投資交易所、女僕教堂 | 全品項 100% | 原價 |
| ⚠️ 野外 | 初始草原、翡翠森林、沿海小徑、搗蛋精靈之森、寵物天堂、舊城邦、世界魔皇巢穴、山林後的花園 | 僅收該節點掉落物 | +20% |
| 🏯 城外安全 | 大士爺廟 | 不收購 | +20% |
...
| 糖果 | 30 | 托幣 | 供奉精靈 |
| 轉法輪 | 500 | 托幣 | 使用後元神 EXP+10 + 修為進度+3 |
...
| 世界魔皇重置 | ① | 管理員手動 | 手動重置 HP 為 5,500,000 |
| 月卡檢查 | ① | 每日 | 檢查並移除過期月卡 |
