...
class Shop(commands.Cog):
    ...
    async def shop(self, interaction: discord.Interaction, category: str = "all"):
        ...
        for r in rows:
            ...
            if d["item_type"] == "material":
                ...
                cons.append(f"{d['emoji']} **{d['name']}**  💰 {d['buy_price']:,} 安幣\n　{d['description']}")
        ...
        if cons and category in ("all", "consumables"):
            embed.add_field(name="🧪 消耗品（用安幣購買，固定價格）", value="\n".join(cons), inline=False)
        embed.set_footer(text="/shop_buy 購買消耗品 | /shop_sell 出售材料")
        await interaction.response.send_message(embed=embed)

    @app_commands.command(name="shop_buy", description="購買消耗品（使用安幣）")
    @app_commands.describe(item_name="要購買的道具", quantity="購買數量")
    @app_commands.choices(item_name=[
        app_commands.Choice(name="體力恢復劑 100安幣", value="體力恢復劑"),
        app_commands.Choice(name="強效體力恢復劑 300安幣", value="強效體力恢復劑"),
        app_commands.Choice(name="線香 50安幣", value="線香"),
        app_commands.Choice(name="糖果 30安幣", value="糖果"),
        app_commands.Choice(name="轉法輪 500安幣", value="轉法輪"),
    ])
    async def shop_buy(self, interaction: discord.Interaction, item_name: str, quantity: int = 1):
        ...
        async with pool.acquire() as db:
            ...
            if user["an_bi"] < total_cost:
                await interaction.response.send_message(
                    f"🔴 安幣不足！需要 {total_cost:,} 元，當前 {user['an_bi']:,} 元。"
                )
                return
            await db.execute(
        ...
        await interaction.response.send_message(
            f"✅ 購買成功！{item['emoji']} **{item_name}** ×{quantity}（花費 {total_cost:,} 安幣）"
        )
